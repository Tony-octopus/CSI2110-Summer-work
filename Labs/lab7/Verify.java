...
public class Verify {
  ...
  public void randomTrees( int _noNodes, int _nTrees ) {
    ...
    for(int i=0; i<_nTrees; ++i) { 
      ...
      //instructions say to generate _nTrees so idk put ts in loop
      TreeMap tree = new TreeMap();
      AVLTreeMap aTree = new AVLTreeMap();
	...
	if ( aTree.get(randKey) == null && tree.get(randKey) == null) { // check if key in tree?
	  // insert into AVL
    aTree.put(randKey, randKey);
    tree.put(randKey, randKey);
	  // insert into BST
      ...
      updateCounters( aTree.height(), tree.height());
    }
    return;
  }

...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
  ...
  private int heightRecursive(Position<Entry<K,V>> root) {
      if (this.isExternal(root)){return 0;}

      return (1 + Math.max(heightRecursive(left(root)), heightRecursive(right(root))));
  }

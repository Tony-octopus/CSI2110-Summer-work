...
public class SequenceAlgs<E> {
	...
	public boolean isPalindrome (Sequence<E> S) {
		...
		// ***** add your code here  *******************
		//insutruction says must use index based access and run in O(n) time

		// LinkedSequence s = (LinkedSequence)S;
		//assuming get() runs in O(n) time even though I know it doesn't
		for (int i = 0; i < S.size()/2; i++){			
			if ( !((S.get(i)).equals(S.get(S.size() -1-i )))  ){
				return (false);
			}
